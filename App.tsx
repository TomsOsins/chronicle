    ...
    <div className="w-screen h-screen flex items-center justify-center p-4 relative overflow-hidden inter">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-md p-4">
           <div className="bg-[#121212] w-full max-w-[400px] p-8 border-t-4 border-[#FF2C2C] relative">
              <div className="mb-6">
              ...
              <p className="mono text-[11px] text-[#F4F1EA]/70 mb-8 uppercase tracking-tight">Purge "{deleteTarget.name}" from history?</p>
              <div className="space-y-3">
                 <button onClick={confirmDelete} className="w-full bg-[#FF2C2C] text-white py-3 text-[11px] font-black uppercase mono">Confirm Erasure</button>
                 <button onClick={() => setDeleteTarget(null)} className="w-full bg-transparent border border-white/20 text-[#F4F1EA]/60 py-3 text-[11px] font-black uppercase mono">Cancel</button>
              </div>
           </div>
        </div>
      )}
      ...
      {isLedgerModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/90 backdrop-blur-md p-4">
          <div className="bg-[#121212] w-full max-w-[360px] p-8 border-t-4 border-[#FF2C2C] relative">
            <div className="flex justify-between items-start mb-8">
              <h2 className="six-caps text-7xl text-[#F4F1EA] leading-tight uppercase">New Ledger</h2>
              <button onClick={() => setIsLedgerModalOpen(false)} className="text-[#FF2C2C] font-black uppercase text-[11px] mono">[ CLOSE ]</button>
            </div>
            <form onSubmit={handleAddLedger} className="space-y-6">
              <input required value={newLedger.name} onChange={e => setNewLedger({...newLedger, name: e.target.value})} className="w-full bg-white/5 border border-white/10 p-4 text-[11px] font-bold uppercase text-[#F4F1EA] focus:outline-none focus:border-[#FF2C2C] mono" placeholder="CHRONICLE ID..." />
              <div className="grid grid-cols-2 gap-4">
                <input value={newLedger.era} onChange={e => setNewLedger({...newLedger, era: e.target.value})} className="w-full bg-white/5 border border-white/10 p-4 text-[11px] font-bold uppercase text-[#F4F1EA] focus:outline-none focus:border-[#FF2C2C] mono" placeholder="4TH ERA" />
                <input value={newLedger.cycle} onChange={e => setNewLedger({...newLedger, cycle: e.target.value})} className="w-full bg-white/5 border border-white/10 p-4 text-[11px] font-bold uppercase text-[#F4F1EA] focus:outline-none focus:border-[#FF2C2C] mono" placeholder="01" />
              </div>
              <button type="submit" className="w-full bg-[#FF2C2C] text-white py-4 text-[11px] font-black uppercase tracking-[0.3em] mono">Authorize Inscription</button>
            </form>
          </div>
        </div>
      )}

      <div className="relative max-w-[1120px] w-full">
        ...
        <div className="w-full h-[720px] rounded-[24px] overflow-hidden shadow-[0_30px_100px_rgba(0,0,0,0.9)] relative z-10 ring-[8px] ring-[#121212] ring-inset bg-black transform translate-z-0 isolation-auto">
          <div className="grid grid-cols-[300px_1fr] h-full w-full">
            <aside className="flex flex-col bg-[#121212] relative border-r-2 border-[#121212] overflow-hidden rounded-l-[18px]">
              <div className="flex-1 bg-[#F4F1EA] flex flex-col pt-14 pb-8 px-8 relative rounded-tl-[18px] overflow-hidden min-h-0">
                <Marker position="tl" /> 
              ...
              <div className={`relative z-40 bg-[#121212] transition-all duration-500 ease-in-out flex flex-col overflow-hidden rounded-bl-[18px] ${isInscriptionExpanded ? 'max-h-[440px]' : 'max-h-[56px]'}`}>
                 <button 
                   ...
                   className="w-full flex justify-between items-center py-4 px-8 shrink-0 hover:bg-white/5 transition-colors"
                 >
                 ...
                 <div className={`px-8 py-4 space-y-4 transition-opacity duration-300 ${isInscriptionExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            ...
            >
              ...
              {selectedCity && !loading && (
                <div className="animate-in fade-in duration-1000">
                  ...
                  <div className="flex justify-between items-center mt-20 border-y border-[#FF2C2C]/40 py-4 px-0">
